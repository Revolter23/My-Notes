...
import { createSession, deleteSession, updateSession } from '@/app/_lib/session';
...
export async function createNote(prevState: State, formData: FormData) {
	...
	try {
		await sql`
			...
			VALUES (${title}, ${content}, ${date}, ${session.userId as string})
  		`;
	} catch (error) {
...
export async function loginAuth(prevState: UserState, formData: FormData) {
